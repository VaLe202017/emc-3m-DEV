...
eEthRes __attribute__((weak)) EthPhyInit(eEthOpenFlags oFlags, eEthPhyCfgFlags cFlags, eEthOpenFlags* pResFlags) {
    ...
    if (cFlags & ETH_PHY_CFG_AUTO) {
        cFlags = hwFlags;
    } else { // some minimal check against the way the hw is configured
        swFlags = cFlags & (ETH_PHY_CFG_RMII | ETH_PHY_CFG_ALTERNATE);

        if ((swFlags ^ hwFlags) != 0) { // hw-sw configuration mismatch MII/RMII, ALT/DEF config
    ...
    if (!_PhyDetectReset()) { // failed to detect the PHY
    ...
    if (oFlags & ETH_OPEN_AUTO) { // advertise auto negotiation
    ...
    if (!(matchCpbl & MAC_COMM_CPBL_MASK)) { // no match?
    ...
    if (matchCpbl & _BMSTAT_AN_ABLE_MASK) { // ok, we can perform auto negotiation
    ...
    } else { // ok, just don't use negotiation
...
eEthRes __attribute__((weak)) EthPhyNegotiationComplete(int waitComplete) {
    ...
    if (phyBMCon.AN_ENABLE) { // just protect from an accidental call
        phyBMCon.w = _PhyReadReg(PHY_REG_BMCON, _PhyAdd);
        ...
        if (waitComplete) {
            ...
            if (phyBMCon.AN_RESTART) { // not started yet
            ...
            if (!phyBMCon.AN_RESTART) { // ok, started
...
eEthLinkStat __attribute__((weak)) EthPhyGetLinkStatus(int refresh) {
    ...
    phyStat.w = _PhyReadReg(PHY_REG_BMSTAT, _PhyAdd);
    if (phyStat.LINK_STAT == 0 && refresh) { // link down could be an old condition. re-read
        phyStat.w = _PhyReadReg(PHY_REG_BMSTAT, _PhyAdd);
    }
...
int __attribute__((weak)) EthPhyReset(int waitComplete) {
    ...
    if (waitComplete) { // wait reset self clear
        __BMCONbits_t bmcon;
        ...
        bmcon.w = _PhyReadReg(PHY_REG_BMCON, _PhyAdd);
        if (bmcon.RESET) { // tmo clearing the reset
            return 0;
        }
    }
...
static int _PhyDetectReset(void) {
    ...
    if (bmcon.RESET) { // that is already suspicios...but give it a chance to clear itself
        tWaitReset = (GetSystemClock() / 2000) * PHY_RESET_CLR_TMO;
        tStart = ReadCoreTimer();
        do {
            bmcon.w = _PhyReadReg(PHY_REG_BMCON, _PhyAdd);
