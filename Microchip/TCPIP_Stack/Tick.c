...
static BYTE vTickReading[6] __attribute__((aligned));
...
void TickInit(void) {
    T4CON = 0x0000U;
    T4CONbits.TCS = 0U;
    T4CONbits.TCKPS = 7; //prescaler 1:256
    PR4 = 0xfffful; //3125ul; //(TICKS_PER_SECOND)/100; // 10ms per tick
    TMR4 = 0x0000U;

    INTClearFlag(INT_T4);
    IPC4bits.T4IP = 2; // Interrupt priority 2 (low)
    IFS0CLR = _IFS0_T4IF_MASK;
    IEC0SET = _IEC0_T4IE_MASK;
    //INTSetVectorPriority(INT_TIMER_4_VECTOR, PRI_LEVEL(SYS_T4_IPL));
    //INTSetVectorSubPriority(INT_TIMER_4_VECTOR, SUBPRI_LEVEL(SYS_T4_SIPL));
...
static void GetTickCopy(void) {
    ...
    do {
        ...
        Nop();
        IEC0CLR = _IEC0_T4IE_MASK; // Disable interrupt
        // Get low 2 bytes
        wTemp.Val = TMR4;
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void __attribute((interrupt(ipl5AUTO), vector(_TIMER_4_VECTOR), nomips16)) _T4Interrupt(void) {
    IFS0CLR = _IFS0_T4IF_MASK;
    dwInternalTicks++;
}
